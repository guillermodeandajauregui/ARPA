...
#resultados <- "data/mock_berlin_2probes.eds"

################################################################################
#wrap it up in a nice, shiny-compliant function
################################################################################

qpcr_pipeline.berlin <- function(input, output){
  ...
  has_allProbes <- CheckProbesEDS(eds = input, my_probes = berlin_probes)
  ...
  #test.results <- test.plate(tdrn = my_deltaRN, probes = berlin_probes)
  
  ################################################################################
  #Plot preparation
  ################################################################################
  
  threshold_list = lapply(X = berlin_probes, 
                          FUN = function(i){
  ...
  test_diagnosis <- berlin_classification(test.results)
