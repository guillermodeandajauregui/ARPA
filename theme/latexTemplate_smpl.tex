...
\huge \bfseries{\scshape{qPCR curves}}\\
\normalsize \bfseries{\scshape{Sample \texttt{$id$}}}
}
\end{center}
$endif$
...
{\fontfamily{cmss}\selectfont
 ...
 Institution code: $if(institution)$\texttt{$institution$}$else$TBD$endif$ & Report date: $if(date)$\texttt{$date$}$else$TBD$endif$\\
Plate ID: $if(plate)$\texttt{$plate$}$else$TBD$endif$ & Plate control: $if(controls)$\texttt{PASS}$else$\cellcolor{red!50}\texttt{FAIL}$endif$\\
	\hline
	\end{tabular*}
}	
\vspace{1mm}
