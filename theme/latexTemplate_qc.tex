...
\huge \bfseries{\scshape{qPCR plate}}\\
\normalsize \bfseries{\scshape{quality report}}
}
\end{center}
$endif$
...
Plate: $if(plate)$\texttt{$plate$}$else$TBD$endif$ & Report date: $if(date)$\texttt{$date$}$else$TBD$endif$\\
 Number of controls: $if(ncontrols)$\texttt{$ncontrols$}$else$TBD$endif$ & Results: $if(controls)$\texttt{PASS}$else$\cellcolor{red!50}\texttt{FAIL}$endif$\\
	\hline
	\end{tabular*}
}	
\vspace{1mm}
