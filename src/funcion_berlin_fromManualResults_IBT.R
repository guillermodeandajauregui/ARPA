...
funcion_berlin_fromManualResults <- function(input_eds, 
  ...
  qc.results <- 
  qc.samples %>% 
  mutate(classification = case_when(sample_name == "NTC" & gen_e == Inf & gen_r_nasa_p == Inf~ "PASS", 
                                    sample_name == "CRE" & gen_e == Inf & gen_r_nasa_p == Inf~ "PASS", 
                                    sample_name == "PTC" & gen_e < 30 & gen_r_nasa_p < 30 ~ "PASS", 
